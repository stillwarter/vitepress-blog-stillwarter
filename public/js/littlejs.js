 ...
 * 如果启用调试，则为True 进入调试状态，这个参数就是ture了
 *  @type {Boolean}
 *  @default
 *  @memberof Debug */
const debug = 1;
 ...
 *  @default
 *  @memberof Debug */
const debugPointSize = 1.5;

/** True if watermark with FPS should be shown, false in release builds
 * 如果应显示带有FPS的水印，则为True，而在发布版本中为false
 *  @type {Boolean}
 *  @default
 *  @memberof Debug */
let showWatermark = 1;
 ...
 * 断言experssion是否为false，在发布版本中不执行任何操作
...
const debugRender = () => {
  ...
  if (debugOverlay) {
    ...
    for (const o of engineObjects) {
      ...
      o.parent &&
        drawLine(o.pos, o.parent.pos, 0.1, new Color(0, 0, 1, 0.5), 0);
    }
 ...
 *LittleJS实用程序类和函数
 *<br>-通用数学库
 *<br>-Vvector2-快速、简单、简单的2D矢量类
 *<br>-Color-包含一些数学函数的rgba颜色
 *<br>-计时器-自动跟踪时间
 *@namespace实用程序
 */
 ...
 * 返回以第二个参数为模（若没有第二个参数，则默认为1）的第一个参数，但进行了调整，使负数按预期工作（模运算）
 ...
 * 启用webgl渲染，可以禁用webgl并从构建中删除（禁用某些功能）
 *  @type {Boolean}
 *  @default
 *  @memberof Settings */
let glEnable = 1;
 ...
 * 如果为true，dpad输入也被路由到左侧模拟棒（以获得更好的访问性）？
 *  @type {Boolean}
 *  @default
 *  @memberof Settings */
 ...
 *<br>-按renderOrder排序
 *<br>-对象可以附加子对象
 *<br>-父级在子级之前更新，并设置子级转换
 *<br>-调用destroy（）以清除对象
 *<br>
 *<br>物体使用的物理系统简单快速，但有一些注意事项。。。
 *<br>-碰撞使用轴对齐的大小，对象的旋转角度仅用于渲染
 *<br>-从物理角度保证对象不会与瓷砖碰撞相交
 *<br>-如果一个对象在瓦片碰撞中启动或移动，它将不会与该瓦片碰撞
 *<br>-对象的碰撞可以设置为实体以阻挡其他对象
 *<br>-物体可能会被推到重叠的其他固体物体中，如果是这样，它们会被推开
 *<br>-实体对象的性能更高，应谨慎使用
 *@示例
 *创建一个引擎对象，通常你会先用自己的扩展类
 *const pos=vec2（2,3）；
 *const object=新的EngineObject（pos）；
 */
...
*<br>-同时提供Canvas2D和WebGL的混合

*<br>-使用WebGL进行超快速瓷砖渲染

*<br>-可以应用旋转、镜像、颜色和附加颜色

*<br>-许多有用的实用程序功能

*<br>

*<br>LittleJS使用了Canvas2D和WebGL的最佳混合渲染解决方案。

*<br>有3个画布/上下文可供绘制。。。

*<br>-mainCanvas-2D背景画布，非WebGL的东西，如瓷砖层，在这里绘制。

*<br>-glCanvas-用于加速的WebGL批处理渲染系统。

*<br>-overlayCanvas-出现在其他2个画布之上的另一个2D画布。

*<br>

<br>WebGL渲染系统速度非常快，但需要注意的是。。。

*<br>-默认设置仅支持1块瓷砖，以支持更多调用glCreateTexture和glSetTexture

*<br>-切换混合模式（添加）或纹理会导致另一个过度昂贵的绘制调用

*<br>-使用renderOrder将附加渲染组合在一起以缓解此问题

*<br>

*<br>LittleJS渲染解决方案非常简单，可以根据您的需要进行调整！
 ...
 * 绘制以世界空间为中心的纹理瓷砖，如果使用WebGL，则应用颜色
...
*字体图像对象-使用图像中的字符在2D画布上绘制文本

*<br>-96个字符（从空格到波浪号）存储在图像中

*<br>-如果未提供，则使用默认的8x8字体
...
*<br>-适用于带有touchGamepadSize的触摸设备的虚拟游戏板
...
const keyIsDown = (key, device = 0) =>
  inputData[device] && inputData[device][key] & 1;
...
///////////////////////////////////////////////////////////////////////////////
// Input update called by engine 引擎调用的输入更新
...
/**
 * LittleJS Audio System LittleJS音频系统
...
function zzfxG(
  ...
  for (
    ...
    f =
      (frequency += slide += deltaSlide) * Math.cos(modulation * tm++); // frequency // modulation
...
*<br>-将瓦片阵列缓存到屏幕外画布以实现快速渲染

*<br>-层数不受限制，根据需要分配画布

*<br>-与EngineObject的接口发生冲突

*<br>-碰撞层与可见层分离

*<br>-建议具有与碰撞匹配的可见层

*<br>-瓷砖层可以通过canvas2d使用其上下文绘制
...
function tileCollisionRaycast(posStart, posEnd, object) {
  ...
  const posDelta = (posEnd = posEnd.floor()).subtract(
...
*let ParticleEmitter=新ParticleEmitter

*(

*pos，0，1，0，500，PI，//pos，角度，emitSize，emitTime，emitRate，emiteCone

*0，vec2（16），//tileIndex，tileSize

*新颜色（1,1,1），新颜色（0,0,0），//colorStartA，colorStartB

*新颜色（1,1,1,0）、新颜色（0,0,0,0），//colorEndA、colorEndB

*2、.2、.2、.1、.05、//particleTime、sizeStart、sizeEnd、particleSpeed、particleAngleSpeed

*.99，1，1，PI，.05，//阻尼，角度阻尼，重力比例，particleCone，渐变速率，

*.5，1//随机性，碰撞，相加，randomColorLinear，renderOrder
...
*<br>-引擎使用的所有webgl都包含在此处

*<br>-对于普通的东西，您不需要查看或调用此文件中的任何内容

*<br>-对于高级内容，有一些辅助函数可以创建着色器、纹理等

*<br>-可以使用glEnable禁用以恢复到2D画布渲染

*<br>-在GPU上批量渲染精灵，获得令人难以置信的快速性能
...
function glDraw(
  ...
  for (
    let i = 6,
      offset = glBatchCount++ * gl_VERTICES_PER_QUAD * gl_INDICIES_PER_VERT;
    i--;
...
function glInitPostProcess(shaderCode, includeOverlay) {
  ...
  if (!shaderCode)
    // default shader
    shaderCode =
      "void mainImage(out vec4 c,vec2 p){c=texture2D(iChannel0,p/iResolution.xy);}";
 ...
 * 使用回调函数启动LittleJS引擎
 *  @param {Function} gameInit        - Called once after the engine starts up, setup the game 引擎启动后调用一次，设置游戏
 *  @param {Function} gameUpdate      - Called every frame at 60 frames per second, handle input and update the game state 以每秒60帧的速度调用每一帧，处理输入并更新游戏状态
 *  @param {Function} gameUpdatePost  - Called after physics and objects are updated, setup camera and prepare for render 在物理和对象更新、设置相机并准备渲染后调用
 *  @param {Function} gameRender      - Called before objects are rendered, draw any background effects that appear behind objects 在渲染对象之前调用，绘制出现在对象后面的任何背景效果
 *  @param {Function} gameRenderPost  - Called after objects are rendered, draw effects or hud that appear above all objects 在渲染对象后调用，绘制出现在所有对象上方的效果或色调
 *  @param {String} [tileImageSource] - Tile image to use, everything starts when the image is finished loading 平铺要使用的图像，当图像加载完成时，一切都开始
 *  @memberof Engine */
function engineInit(
  gameInit,
  gameUpdate,
